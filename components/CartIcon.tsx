...
export default function CartIcon({ isTransparent = false }: CartIconProps) {
  const { totalItems, openCart: openAddOnsCart } = useCart();
  const { getItemCount, openCart: openYachtCart } = useCartStore();
  
  const yachtCount = getItemCount();
  const totalCount = totalItems + yachtCount;
  
  // If there are yacht bookings, open yacht cart, otherwise add-ons cart
  const handleClick = () => {
    if (yachtCount > 0) {
      openYachtCart();
    } else {
