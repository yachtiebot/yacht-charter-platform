...
export default function TestSpiralsPage() {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const { addItem } = useCart();

  const handleCustomizationSubmit = (customization: SpiralCustomization) => {
    addItem({
      id: 'gourmet-spirals',
      name: 'Gourmet Spirals Platter',
      price: 89.99,
      category: 'catering',
    ...
    <div className="min-h-screen bg-[#faf9f7] flex items-center justify-center p-8">
      <div className="text-center max-w-xl">
        ...
        <p className="text-sm text-[#6b6b6b] mb-10 px-4" style={{ fontWeight: 300 }}>
          Choose one of five artisan combinations
        </p>
      ...
      <SpiralCustomizationModal
        ...
        productName="Gourmet Spirals Platter"
        productPrice={89.99}
      />
    </div>
  );
