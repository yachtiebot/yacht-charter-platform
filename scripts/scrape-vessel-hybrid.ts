#!/usr/bin/env npx tsx
/**
 * HYBRID Vessel Scraper - Scrapes website data + uses PDF images
 * 
 * Usage: npx tsx scripts/scrape-vessel-hybrid.ts <squarespace-url> [--pdf]
 * Example: npx tsx scripts/scrape-vessel-hybrid.ts https://www.miamiyachtingcompany.com/monterey-black --pdf
 * 
 * With --pdf flag: Extracts images from Dropbox /PDF/{Yacht-ID}.pdf
 * Without --pdf: Uses images from website (original behavior)
 */
...
import { extractImagesFromPdf, cleanupPdfTemp } from './lib/pdf-extractor';
...
async function main() {
  ...
  const usePdf = process.argv.includes('--pdf') || process.argv.includes('--use-pdf');
  
  if (!url) {
    console.error('❌ Usage: npx tsx scripts/scrape-vessel-hybrid.ts <url> [--pdf]');
    console.error('');
    console.error('Options:');
    console.error('  --pdf    Use images from Dropbox /PDF/{Yacht-ID}.pdf instead of website');
    process.exit(1);
  }
  
  console.log('🚢 Miami Yachting Company - Hybrid Vessel Scraper\n');
  if (usePdf) {
    console.log('📄 PDF Mode: Will extract images from Dropbox PDF if available\n');
  }
  ...
  try {
    ...
    // Step 2: Process images (PDF or website)
    let imageFiles: string[] = [];
    
    if (usePdf) {
      // Try to extract from PDF first
      imageFiles = await extractImagesFromPdf(vessel.yachtId);
      
      if (imageFiles.length === 0) {
        console.log('   ⚠️  No PDF found, falling back to website images...');
      }
    }
    
    if (imageFiles.length === 0) {
      // Use website images (original behavior)
      console.log(`\n📸 Processing ${vessel.imageUrls.length} images from website...`);
      ...
      // Use PDF images
      console.log(`\n📸 Processing ${imageFiles.length} images from PDF...`);
      
      for (let i = 0; i < imageFiles.length; i++) {
        try {
          const isHero = i === 0;
          const filename = isHero 
            ? `Miami_Yachting_Company_${vessel.yachtId}_hero.webp`
            : `Miami_Yachting_Company_${vessel.yachtId}_${String(i).padStart(2, '0')}.webp`;
          ...
          while (quality >= 60) {
            buffer = await sharp(imageFiles[i])
              .webp({ quality })
              .toBuffer();
            ...
            if (sizeKB <= 500) {
              console.log(`     ✅ ${sizeKB.toFixed(0)}KB (q:${quality})`);
              break;
            }
          ...
          const { error } = await supabase.storage
            .from('yacht-photos')
            .upload(`${vessel.yachtId}/${filename}`, buffer!, {
              contentType: 'image/webp',
              upsert: true
            });
      ...
      // Add image 15 (copy of hero) for consistency
      if (imageFiles.length === 15) {
        console.log(`  Adding image ${imageFiles.length} (copy of hero)...`);
        const { data: hero } = await supabase.storage
          .from('yacht-photos')
          .download(`${vessel.yachtId}/Miami_Yachting_Company_${vessel.yachtId}_hero.webp`);
        
        await supabase.storage
          .from('yacht-photos')
          .upload(`${vessel.yachtId}/Miami_Yachting_Company_${vessel.yachtId}_15.webp`, hero!, {
            contentType: 'image/webp',
            upsert: true
          });
        console.log(`     ✅ Created image 15`);
      }
      
      console.log(`\n✅ Uploaded ${imageFiles.length} images from PDF to yacht-photos/${vessel.yachtId}/`);
      
      // Cleanup PDF temp files
      cleanupPdfTemp(vessel.yachtId);
    }
    ...
    const totalImages = imageFiles.length > 0 ? imageFiles.length : vessel.imageUrls.length;
    const galleryCount = totalImages - 1;
