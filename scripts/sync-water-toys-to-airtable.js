...
/**
 * Sync Water Toys from website to Airtable
 * Run: node scripts/sync-water-toys-to-airtable.js
 */
...
async function syncToAirtable() {
  if (!AIRTABLE_API_KEY || !AIRTABLE_BASE_ID) {
    console.error('❌ Missing environment variables: AIRTABLE_API_KEY or AIRTABLE_BASE_ID');
    process.exit(1);
  }

  console.log(`🚀 Syncing ${waterToysData.length} water toys to Airtable...`);
    ...
    try {
      ...
      if (searchData.records && searchData.records.length > 0) {
        ...
        if (updateResponse.ok) {
          ...
          console.log(`❌ Error updating ${toy.name}:`, error.error);
        ...
        if (createResponse.ok) {
          console.log(`✨ Created: ${toy.name}`);
          results.created.push(toy.name);
        } else {
          const error = await createResponse.json();
          console.log(`❌ Error creating ${toy.name}:`, error.error);
          results.errors.push({ name: toy.name, error: error.error });
        }
      }
      ...
      console.log(`❌ Exception for ${toy.name}:`, error.message);
